...
try {
    ...
    $stmtCliente->execute([
        $data['cliente']['nome'], $data['cliente']['telefone'], $data['cliente']['email'],
        $data['cliente']['rua'], $data['cliente']['numero'], $data['cliente']['bairro'],
        $data['cliente']['cidade'], $data['cliente']['estado'], $data['cliente']['cep']
    ]);
    ...
    if (!empty($data['pets']) && is_array($data['pets'])) {
        ...
        foreach ($data['pets'] as $pet) {
            $stmtPet->execute([
                $clienteId, $pet['nome'], $pet['especie'], $pet['raca'],
